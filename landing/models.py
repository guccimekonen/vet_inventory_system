...
class Shipment(models.Model):
    ...
    # ===== COST COMPONENTS =====
    insurance = models.DecimalField(max_digits=20, decimal_places=2, default=0)  # Already in ETB
    ...
    def get_total_fob_birr(self):
        return self.get_total_fob() * self.exchange_rate

    def get_insurance_birr(self):
        # ✅ Already in ETB, no conversion
        return self.insurance or Decimal('0')

    def get_freight_birr(self):
        return (self.freight_documentation or Decimal('0')) * self.exchange_rate

    def get_cif_birr(self):
        # ✅ CIF = FOB(ETB) + Freight(ETB) + Insurance(ETB)
    ...
    def save(self, *args, **kwargs):
        ...
        # ✅ Custom duty = 15% of CIF
        self.custom_duty_amount = (self.custom_duty_percent / Decimal('100')) * cif_birr

        # ✅ Total additional cost = custom duty + all costs (insurance already ETB)
        ...
        super().save(update_fields=['custom_duty_amount', 'total_additional_cost'])


# =========================
# SHIPMENT ITEM (BATCH SYSTEM)
# =========================
class ShipmentItem(models.Model):
    ...
    unit_landed_cost = models.DecimalField(max_digits=20, decimal_places=4, blank=True, null=True)
    ...
    def get_fob_value_birr(self):
        return self.get_fob_value() * self.shipment.exchange_rate

    def get_unit_price_birr(self):
        return self.unit_price * self.shipment.exchange_rate

    def get_suggested_selling_price(self):
        """
        Suggested selling price in ETB (30% margin default)
        """
    ...
    def save(self, *args, **kwargs):
        ...
        if not shipment.total_additional_cost:
            return
        ...
        for item in items:
            ...
            item.cost_share_percent = (item.fob_value / total_fob * Decimal('100')) if total_fob > 0 else Decimal('0')
            item.allocated_cost = (item.cost_share_percent / Decimal('100')) * shipment.total_additional_cost

            # ✅ Correct unit landed cost calculation
            item.total_cost = item.get_fob_value_birr() + (item.allocated_cost or Decimal('0'))

            item.unit_landed_cost = item.total_cost / item.quantity if item.quantity > 0 else Decimal('0')

            super(ShipmentItem, item).save(update_fields=[
                'fob_value',
                'cost_share_percent',
                'allocated_cost',
                'total_cost',
